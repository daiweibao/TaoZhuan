...
#pragma clang diagnostic pop
                            ...
                            if (self.verboseLogging)
                            {
                                ...
                                {
                                    NSLog(@"当前在iTunes上没有找到一个新版本的应用程序。当前版本是%@,最新版本是%@",self.applicationVersion, latestVersion);
...
#pragma mark ========= 自己修改 =========================
    //分割跟新内容，只显示最近更新，这里用 lastObject firstObject代码不会崩溃
    NSArray * marray1 = [details componentsSeparatedByString:@"本次更新："];
    NSArray * marray2 = [[marray1 lastObject] componentsSeparatedByString:@"最近更新："];
    //得到版本内容
    details = [marray2 firstObject];
    ...
    return [AlertViewTool AlertAlertWithTitle:title Message:details otherItemArrays:@[@"暂不更新",@"立即更新"] viewController:topController handler:^(NSInteger index) {
        if (index==0) {
            //暂不更新
        }else if (index==1){
            //立即更新
            //只需要修改后面的App-Id即可--去appstroe下载嗅美
