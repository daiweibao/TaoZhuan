...
- (void)drawXYLine{
    ...
    layer.strokeColor = [UIColor clearColor].CGColor;//坐标轴的颜色
...
- (void)drawLabels{
    ...
    for(int i = 1; i <= count; i ++){
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake(kMargin + everyX * i - everyX / 2, CGRectGetHeight(myFrame) - kMargin+10, everyX, kMargin)];
        
        lbl.textColor = MAIN_COLOR_GRAY;
        lbl.font = [UIFont systemFontOfSize:12];
        //lbl.backgroundColor = [UIColor brownColor];
        ...
        case PointType_Circel:
            for (int i = 0; i < count; i ++) {
                ...
                UIBezierPath *path = [UIBezierPath
                                      ...
                                      //    方法2
                                      bezierPathWithRoundedRect:CGRectMake(point.x - 10.0*px, point.y - 10.0*px, 20.0*px, 20.0*px) cornerRadius:10.0*px];
                
                
                CAShapeLayer *layer = [CAShapeLayer layer];
                layer.path = path.CGPath;
                layer.strokeColor = MAIN_COLOR.CGColor;
                layer.fillColor = MAIN_COLOR.CGColor;
                [self.bgView.layer addSublayer:layer];
            }
...
- (void)drawValues{
    for (int i = 0; i < count; i ++) {
        CGPoint nowPoint = CGPointMake(kMargin + everyX * (i + 1), kMargin + (1 - [self.yValues[i] floatValue] / maxY) * allH);
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake(nowPoint.x - everyX/2.0-5, nowPoint.y - 20-2, everyX+10, 20)];
