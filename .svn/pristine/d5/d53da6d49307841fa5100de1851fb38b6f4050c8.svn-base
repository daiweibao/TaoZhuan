...
-(void)createUI{
    ...
    //从本地取出账号
    phoneTextField.text = [[NSUserDefaults standardUserDefaults] objectForKey:@"MobilePhoneNumber"];
    ...
    passwordTextField.secureTextEntry = YES;//默认密文
    ...
    [loginButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(passwordTitleBution.mas_bottom).mas_offset(40);
    ...
    labelTips.textColor  =[UIColor blueColor];
    labelTips.textAlignment = NSTextAlignmentCenter;
    labelTips.layer.borderWidth = 1;
    labelTips.layer.borderColor = [UIColor blueColor].CGColor;
    labelTips.layer.cornerRadius = 3;
    labelTips.userInteractionEnabled = YES;
    ...
    [labelTips addTapActionTouch:^{
        //注册界面
        RegisteredController * registVC = [[RegisteredController alloc]init];
...
-(void)loginButtonClick:(UIButton*)button{
    ...
    [MBProgressHUD showHUDLodingStart:@"登录中" toView:self.view];
    //登陆
    NSDictionary * dict = @{@"userName":self.phoneTextField.text,//用户名
                            @"password":[NSString md5:self.passwordTextField.text]//密码，md5加密
                            };
    [GXJAFNetworking POST:PHONE_LOGIN parameters:dict success:^(id  _Nullable responseObject) {
