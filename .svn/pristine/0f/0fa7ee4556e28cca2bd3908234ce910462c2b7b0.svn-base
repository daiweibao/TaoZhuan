...
- (void)viewDidLoad {
    ...
    if ([NSString isNULL:self.titleNavStr]) {
        ...
        self.titleLabel.text = self.titleNavStr;
    }
    
    //判断html为空
    if ([NSString isNULL:self.htmlString]) {
        //网页不能打开
        [AlertGXJView AlertGXJAlertWithController:self Title:nil Message:@"该网页已失效~" otherItemArrays:@[@"知道啦"] Width:-1 Type:-1 handler:^(NSInteger index) {
            if (index==0) {
                //返回
