...
@implementation SYChineseToPinyin
...
@property (nonatomic, copy) NSString *originalChinese;
@property (nonatomic, copy) NSString *formatChinese;
@property (nonatomic, copy) NSString *pinyin;
@property (nonatomic, copy) NSString *indexLetter;
@end

@implementation SYWord
@end

@interface SYPinyinSort ()
@property (nonatomic, strong) NSMutableArray *words;
@property (nonatomic, copy) NSArray *originalChineses;
@property (nonatomic, copy) NSArray *formatChineses;
@property (nonatomic, copy) NSArray *indexArray;
@property (nonatomic, copy) NSDictionary *cityDicts;
@end

@implementation SYPinyinSort

- (void)setChineses:(NSArray *)chineses {
    if (!chineses || chineses.count == 0 || chineses == _chineses) return;
    _chineses = chineses;
    [self format];
    [self sort];
}
...
- (void)format {
    ...
    [_chineses enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        SYWord *word = [[SYWord alloc] init];
        word.originalChinese = obj ? obj : @"";
        NSString *formatString = [obj stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
        NSCharacterSet *set = [NSCharacterSet characterSetWithCharactersInString:@"@／：；（）¥「」＂、[]{}#%-*+=_\\|~＜＞$€^•'@#$%^&*()_+'\""];
        word.formatChinese = [formatString stringByTrimmingCharactersInSet:set];

        //判断首字符是否为字母
        NSString *regex = @"[A-Za-z]+";
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
        NSString *initialString = [word.formatChinese length] ? [[word formatChinese] substringToIndex:1] : @"";

        if ([predicate evaluateWithObject:initialString]) {
            word.pinyin = [word.formatChinese capitalizedString];
            word.indexLetter = [initialString capitalizedString];
        }else {
            if (![word.formatChinese isEqualToString:@""]) {
                word.pinyin = [SYChineseToPinyin pinyinFromChiniseString:word.formatChinese];
                NSString *firstLetter = [word.pinyin substringToIndex:1];
                if ([predicate evaluateWithObject:firstLetter]) {
                    word.indexLetter = firstLetter;
                } else {
                    word.indexLetter = @"#";
                }
            }else {
    ...
    NSSortDescriptor *sort = [NSSortDescriptor sortDescriptorWithKey:@"indexLetter" ascending:YES];
    NSSortDescriptor *sort2 = [NSSortDescriptor sortDescriptorWithKey:@"pinyin" ascending:YES];
    [words sortUsingDescriptors:[NSArray arrayWithObjects:sort, sort2, nil]];
    self.words = words;
}

- (void)sort {
    __block NSMutableArray *indexArray = @[].mutableCopy;
    __block NSMutableArray *originalChineses = @[].mutableCopy;
    __block NSMutableArray *formatChineses = @[].mutableCopy;

    __block NSMutableArray *originalItem = @[].mutableCopy;
    __block NSMutableArray *formatItem = @[].mutableCopy;
    __block NSString *tempString = @"";

    [self.words enumerateObjectsUsingBlock:^(SYWord *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (![tempString isEqualToString:obj.indexLetter]) {

            originalItem = [NSMutableArray array];
            [originalItem addObject:obj.originalChinese];
            [originalChineses addObject:originalItem];

            formatItem = [NSMutableArray array];
            [formatItem addObject:obj.formatChinese];
            [formatChineses addObject:formatItem];

            [indexArray addObject:obj.indexLetter];

            tempString = obj.indexLetter;
        }else {
            [originalItem addObject:obj.originalChinese];
            [formatItem addObject:obj.formatChinese];
        }
    ...
    self.originalChineses = originalChineses;
    self.formatChineses = formatChineses;
    self.cityDicts = [NSDictionary dictionaryWithObjects:self.originalChineses forKeys:indexArray];
}

+ (instancetype)defaultPinyinSort {
    static SYPinyinSort *_s = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _s = [[SYPinyinSort alloc] init];
    });
    return _s;
}

+ (NSArray *)sortWithChineses:(NSArray *)chineses {
    [[self defaultPinyinSort] setChineses:chineses];
    return [[self defaultPinyinSort] originalChineses];
}

+ (NSDictionary *)dictWithChineses:(NSArray *)chineses {
    [[self defaultPinyinSort] setChineses:chineses];
    return [[self defaultPinyinSort] cityDicts];
}
