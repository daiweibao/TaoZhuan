...
- (instancetype)init{
    ...
    if (self) {
         ...
         [Network getAppAdAFN];//请求启动广告，缓存下来
...
-(void)example01{
    ...
    [XHLaunchAd setWaitDataDuration:4];//这种情况很少会触发，网络请求哪里已做处理。（这次修改后用读取缓存的方法）

    //广告数据请求
    [Network getLaunchAdImageDataSuccess:^(NSDictionary * response) {
//        NSLog(@"广告数据 = %@",response);
        if ([NSString isNULL:response[@"reply"][@"imgUrl"]]) {
        //如果没有启动页就移除广告进入首页
            [self skipAction];
            return;
        }
...
//        LaunchAdModel *model = [[LaunchAdModel alloc] initWithDict:response[@"data"]];
        ...
        imageAdconfiguration.imageNameOrURLString =[NSString stringWithFormat:@"%@",response[@"reply"][@"imgUrl"]];
        ...
        //广告点击打开链接，json字符串传进去，点击时在解析成字典
        imageAdconfiguration.openURLString = [NSDictionary dictionaryToJson:response[@"reply"]];
        //广告显示完成动画
        imageAdconfiguration.showFinishAnimate =ShowFinishAnimateLite;
        //广告显示完成动画时间
        imageAdconfiguration.showFinishAnimateTime = 0.8;
        //跳过按钮类型SkipTypeRoundProgressText  SkipTypeTimeText
        imageAdconfiguration.skipButtonType = SkipTypeTimeText;
        //后台返回时,是否显示广告
        imageAdconfiguration.showEnterForeground = NO;
        
        //图片已缓存 - 显示一个 "已预载" 视图 (可选)
        if([XHLaunchAd checkImageInCacheWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",response[@"reply"][@"imgUrl"]]]]){
            //设置要添加的自定义视图(可选)
...
- (void)xhLaunchAd:(XHLaunchAd *)launchAd clickAndOpenURLString:(NSString *)openURLString{
    ...
    //调用封装的方法,跳转广告详情
    [AppPushView getAdsafeAction:[NSDictionary jsonToDictionaryWithJsonString:openURLString]];
