...
static CGFloat toolsViewHeith = 48;
...
- (void)canselButtonClick{
    ...
    //过滤重复数据,如：北京市北京市朝阳区  过滤为：北京市朝阳区
    if ([self.province isEqual:self.city]) {
        //删除前面重复的省
        self.province = @"";
    }
...
- (UIView *)toolsView{
    ...
    if (!_toolsView) {
        ...
        _toolsView.backgroundColor = MAIN_COLOR;
    }
    return _toolsView;
}
...
- (UIButton *)canselButton{
    if (!_canselButton) {
        _canselButton = ({
            UIButton *canselButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 58, toolsViewHeith)];
...
- (UIButton *)sureButton{
    if (!_sureButton) {
        _sureButton = ({
            UIButton *sureButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 58, 0, 58, toolsViewHeith)];
