//
//  NSString+PhoneVerify.m
//  ZuiMeiXinNiang
//
//  Created by 爱恨的潮汐 on 16/4/11.
//  Copyright © 2016年 zmxn. All rights reserved.
//

#import "NSString+PhoneVerify.h"
#import "WebViewController.h"
#import <CommonCrypto/CommonDigest.h>//sha1加密需要导入的头文件
//调用系统震动和声音
#import <AudioToolbox/AudioToolbox.h>
@implementation NSString (PhoneVerify)
...
+(CGSize)sizeMyStrWith:(NSString*)text andFontSize:(CGFloat)fontsize andMineWidth:(CGFloat)Width

{
    ...
    if ([text isKindOfClass:[NSString class]] && text) {
       ...
       size = [text boundingRectWithSize:CGSizeMake(Width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:fontsize]} context:nil].size;
    }
    return size;
...
//毫秒转日期（传入NSNumber）
+(NSString * )dateChange:(NSNumber * )dateNum andFormat:(NSString*)formatStr{
    //格式为 yyyy-MM-dd HH:mm:ss
    //毫秒转日期
    NSString * stareStr = [NSString stringWithFormat:@"%@",dateNum];
    double time= [stareStr doubleValue];
...
//统计淘宝链接打开次数
/**
 @param taobaoid 淘宝商品id
 */
+(void)totalTaobaoUrl:(NSString *)taobaoid{
...
/**
 ...
 @param userId 用户ID
 @return 判断结果，YES是自己，NO是别人
 */
+(BOOL)isUserMySelfUserId:(NSString *)userId{
    if ([NSString isNULL:SESSIONID]==NO) {
        //我自己的ID(只有用户登录了才对比)
        if ([[NSString stringWithFormat:@"%@",userId] isEqual: [NSString stringWithFormat:@"%@",USERID]]) {
            //自己
            return YES;
        }else{
            //别人
