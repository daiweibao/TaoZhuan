...
//背景色
#define SYBackgroundColor [UIColor whiteColor]
//#define SYIndexColor SYColor(0, 188, 165)
//右边小字母的颜色
#define SYIndexColor [UIColor colorWithHexString:@"#eb4c97"]
...
- (void)initialize {
    ...
    _textLabel.textColor = [UIColor colorWithHexString:@"#313131"];
...
- (void)layoutSubviews {
    [super layoutSubviews];
    self.textLabel.frame = CGRectMake(0, 0, self.contentView.bounds.size.width-3, self.contentView.bounds.size.height-10);
}
...
- (instancetype)initWithSearchResultsController:(UIViewController *)searchResultsController {
    if (self = [super initWithSearchResultsController:searchResultsController]) {
        ...
        //设置背景图是为了去掉上下黑线
        self.searchBar.backgroundImage = [UIImage imageWithColor:[UIColor groupTableViewBackgroundColor]];
         // 设置SearchBar的颜色主题为白色
...
- (void)setNavigation {
    ...
    //标题
    UILabel *navTitle = [[UILabel alloc]initWithFrame:CGRectMake(44, 20, SCREEN_WIDTH-88, 44)];
    navTitle.text = @"请选择城市";
    ...
    //地区筛选按钮
    CGFloat buttonWindth = SCREEN_WIDTH/3.0;
    UIButton * buttonRight = [UIButton buttonWithType:UIButtonTypeCustom];
    buttonRight.frame = CGRectMake(SCREEN_WIDTH-buttonWindth-15,20,buttonWindth,44);
    [buttonRight setImage:[UIImage imageNamed:@"筛选_3.3"] forState:UIControlStateNormal];
    [buttonRight setTitle:self.currentCityName forState:UIControlStateNormal];
    [buttonRight setTitleColor:[UIColor colorWithHexString:@"#303030"] forState:UIControlStateNormal];
    buttonRight.titleLabel.font = [UIFont systemFontOfSize:12];
    buttonRight.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
  
    // 相比较原来的位置，文字相对左边的距离减少了图片的宽度，右边距离增加了图片的的宽度
    buttonRight.titleEdgeInsets = UIEdgeInsetsMake(0, -buttonRight.imageView.frame.size.width, 0, buttonRight.imageView.frame.size.width);
    // 相比较原来的位置，图片距离左边的距离增加了文字的宽度，右边距离减少了文字的宽度
    buttonRight.imageEdgeInsets = UIEdgeInsetsMake(0, buttonRight.titleLabel.frame.size.width, 0, -buttonRight.titleLabel.frame.size.width);
    
    
    //添加
    [TopView addSubview:buttonRight];
...
- (void)setHotCitys:(NSArray *)hotCitys {
    ...
    _hotCitys = hotCitys;
    if (_cityDicts) {
        [_cityDicts setObject:hotCitys forKey:@"热"];
    }
}
