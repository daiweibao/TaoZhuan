...
@interface TodayEarningsController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation TodayEarningsController
...
- (void)viewDidLoad {
    ...
    //登陆成功后通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(loginSuccessRefresh) name:@"mineLoginSussce" object:nil];
    ...
    [GXJAFNetworking POST:getWeekGain parameters:dic success:^(id  _Nullable responseObject) {
        if ([responseObject[@"code"] isEqual:@"00"]){
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [self.tableView endRefresh_DWB];//结束刷新
            });
        }else{
            //其他错误提示
...
- (UITableView *)tableView {
    if (!_tableView) {
        _tableView=[[UITableView alloc]initWithFrame:CGRectMake(0, MC_NavHeight, SCREEN_WIDTH, SCREEN_HEIGHT-MC_TabbarHeight-MC_TabbarHeight) style:UITableViewStylePlain];
