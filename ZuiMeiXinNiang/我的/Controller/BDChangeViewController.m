...
@interface BDChangeViewController ()<UITextFieldDelegate>{
...
@implementation BDChangeViewController
...
- (void)infoAction:(NSNotification*)notf{
    ...
    //确定键变颜色
    if (self.phoneText.text.length&&self.VerCode.text.length>0) {
...
-(void)createUI{
    //创建父视图
    UIView * loginSub = [[UIView alloc]initWithFrame:CGRectMake(0,64 + 20.0*px, SCREEN_WIDTH, 182.0*px)];
        ...
        if (isPhone==YES) {
            ...
            [GXJAFNetworking POST:SENDER_CODE parameters:dic success:^(id  _Nullable responseObject) {
                if ([responseObject[@"code"] isEqual:@"00"]) {
                    ...
                    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示"  message:responseObject[@"errorMsg"] preferredStyle:UIAlertControllerStyleAlert];
                    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:nil];
                    [alertController addAction:okAction];
                    [self presentViewController:alertController animated:YES completion:nil];
...
-(void)login:(UIButton*)button{
    ...
    //发送验证码
    NSDictionary * dic = @{@"mobile":self.phoneText.text,@"sessionId":SESSIONID,@"code":self.VerCode.text};
    [GXJAFNetworking POST:Phone_Change_Binding parameters:dic success:^(id  _Nullable responseObject) {
        if ([responseObject[@"code"] isEqual:@"00"]) {
            if (ios8orLater) {
        ...
        if ([responseObject[@"code"] isEqual:@"02"]) {
            ...
            if (ios8orLater) {
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示"  message:@"该手机已绑定其他用户" preferredStyle:UIAlertControllerStyleAlert];
                UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:nil];
                [alertController addAction:okAction];
                [self presentViewController:alertController animated:YES completion:nil];
            }else{
                
                UIAlertView * aler = [[UIAlertView alloc]initWithTitle:@"提示" message:@"该手机已绑定其他用户" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                [aler show];
