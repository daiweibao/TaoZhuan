...
#import "TotalGetMoneyController.h"
#import "GetMoneyListCell.h"
#import "TotalGetMoneyModel.h"
#import "HuaShenMoneyBarController.h"

@interface TotalGetMoneyController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation TotalGetMoneyController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.backButton.hidden = NO;
    self.titleLabel.text = @"累计收益";
...
//数据请求
-(void)loadData{
    NSDictionary * dict=@{@"sessionId":SESSIONID,@"page.count":@(50),@"page.currPage":@(_currentPage),@"type":@(1)};
        ...
        if ([responseObject[@"code"] isEqual:@"00"]) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (self.dataSouce.count == 0) {//空白页
                    ...
                    UIView *bg = [[UIView alloc]initWithFrame:CGRectMake(0, MC_NavHeight, SCREEN_WIDTH, SCREEN_HEIGHT-MC_NavHeight)];
                    ...
                    blankPagesTitle.text = @"您暂时还没有收益哦~";
...
- (UIView *)header{
    if (_header == nil) {
        ...
        _header = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 100)];
        _header.backgroundColor = [UIColor whiteColor];
        
        UILabel *moneyLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 100)];
